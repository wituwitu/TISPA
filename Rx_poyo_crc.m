...
%      senal = getaudiodata(grabacion);        %Llena el vector grabacion con la informacion grabada
    ...
    if decode_txt(dest_port, dest_port_length) == '1'
        seq_num = paquete((dest_port_length)*Largo:(dest_port_length)*Largo+seq_num_length*Largo);
    
        disp(strcat('Número de secuencia: ', decode_txt(seq_num, seq_num_length)));
    
        checksum = paquete((seq_num_length+1)*Largo:(seq_num_length+1)*Largo+checksum_length*Largo);
    
        disp(strcat('Checksum recibido: ', decode_txt(checksum, checksum_length)));
    
        data = paquete((seq_num_length+1)*Largo+checksum_length*Largo + 2:end);
        ...
        checksum = decode_txt(checksum, checksum_length);
        checksum_rec01 = Simulink.getFileChecksum('pkt_rec.wav');
        disp(strcat('Checksum calculado: ', checksum_rec01))

        if checksum == checksum_rec01
            ...
            if length(original) == length(recibido) && sum(original~=recibido) < 100
                ...
                all_data = [all_data; data];
                pause(3);
            else disp('Error al recibir el paquete.');
            end
        end
    else
        i = i+1;
